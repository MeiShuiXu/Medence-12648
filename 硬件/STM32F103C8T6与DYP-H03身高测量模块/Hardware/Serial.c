...
void Serial2_Init(void)
{
    ...
    USART_Init(USART2, &USART_InitStructure);               // 配置USART2
...
/**
  * 函    数：UART2发送十进制数字
  * 参    数：Number 要发送的数字（0~4294967295）
  * 参    数：Length 数字的十进制位数（0~10）
  * 返 回 值：无
  */
void Serial2_SendNumber(uint32_t Number, uint8_t Length)
{
    uint8_t i;
    for (i = 0; i < Length; i++) {
        Serial2_SendByte(Number / Serial_Pow(10, Length - i - 1) % 10 + '0'); // 发送每位数字
    }
}
/**
  * 函    数：串口发送一个字符串
